...
`CredentialRedactor` is applied before API problem details, persisted failure text, audit details, Log4j2/Sentry output, and options-file diagnostic output. Security tests cover prefixed configuration names, DSN-like values, and connection-string forms. Generated context follows the same rule and records only paths or finding types.
