...
Tasks report watermark candidates; the run reduces them and the managed service commits the value only after the sink merge succeeds. Failed or cancelled runs do not advance the committed boundary. Bind order and pagination arguments remain manager-specific.
