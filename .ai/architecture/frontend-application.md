...
The SPA uses React Router, MUI, Axios, and TanStack Query. `src/api` owns the credentialed `/api/v1` client and endpoint modules; `src/auth` owns session state; `src/router` owns `ProtectedRoute` and `RequireRole`; `src/pages` composes operational, job-editor, run, user, and permission screens. The generated schema now includes optional retry-policy inputs, resolved policy outputs, and run `availableAt`.
...
`src/api/schema.ts` is generated from the server OpenAPI document. Frontend tests use Testing Library, fresh query clients, and matching router context for parameterized pages. Lease tokens remain internal to managed execution and are asserted absent from the schema and UI. Retry-policy forms preserve explicit complete-mode choices, while run views show retry eligibility without exposing lease identity.

Reference implementations: `replicadb-server/frontend/src/router/routes.tsx`, `src/pages/JobFormPage.tsx`, `src/pages/UsersPage.tsx`, and `src/api/schema.ts`.
