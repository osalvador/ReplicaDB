...
- `replicadb-server` is a sibling Spring Boot module. Its `api` profile exposes REST, runs Quartz scheduling, persists control-plane state, and delegates execution to the root core. A distributed worker profile is a future direction, not an observed runtime.
...
The dependency direction is CLI/options -> core orchestration -> manager adapters. Managed server domain, persistence, API, security, and execution packages surround a translation boundary that produces `ToolOptions`; controllers do not call vendor managers directly. The frontend calls endpoint modules through a credentialed API client.
