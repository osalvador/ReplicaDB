...
Decision: generate `src/api/schema.ts` from Springdoc OpenAPI, alias its component schemas in endpoint modules, and test live or committed schema stability. DTOs explicitly represent lower-case modes, nullable wire values, CSRF responses, retry-policy inputs/outputs, run `availableAt`, and deterministic property order where required. `leaseToken` remains an internal persistence/execution field and must not enter the public schema.
