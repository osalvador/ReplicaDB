...
async function fixture() {
  ...
  for (const directory of ['pagefind', 'server', 'api', 'cli', 'connectors', 'operations', 'reference', 'docs', 'wizard', 'markdown']) await mkdir(join(root, directory), { recursive: true });
  for (const [path, title] of [['index.html', 'Home'], ['server/index.html', 'Server'], ['server.html', 'Legacy server'], ['docs/docs.html', 'Legacy docs'], ['docs/user-guide.html', 'Legacy guide'], ['404.html', '404'], ['api/index.html', 'API index'], ['cli/index.html', 'CLI'], ['connectors/index.html', 'Connectors'], ['operations/index.html', 'Operations'], ['reference/index.html', 'Reference'], ['wizard/index.html', 'Wizard'], ['markdown/converter.html', 'Markup']]) await writeFile(join(root, path), `<title>${title}</title>`);
...
test('validator accepts a complete fixture and rejects broken links', async () => {
  ...
  assert.equal(validateBuiltDocs(root).pages, 13);
...
test('validator rejects a broken local asset', async () => {
  const root = await fixture();
  await writeFile(join(root, 'index.html'), '<title>Home</title><img src="/ReplicaDB/_astro/missing.png" alt="Missing">');
  assert.throws(() => validateBuiltDocs(root), /Broken built links:[\s\S]*missing\.png/);
});
