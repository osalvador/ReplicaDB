...
Grant only the action needed for the resource. `EXECUTE` does not imply
...
If a save conflicts or the resource disappears, refresh the permissions page
before retrying. Review the resulting administrative event in [audit](/ReplicaDB/server/audit/)
and use [errors and empty states](/ReplicaDB/server/errors-and-empty-states/)
to interpret unauthorized or conflict responses.
