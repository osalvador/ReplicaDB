...
create_job() {
    ...
    payload=$(jq -n \
        ...
        '{name: $name, sourceDatasourceId: $source_datasource_id, sourceDatasourceUseEnabled: true, sourceTable: "phase3_source", sourceColumns: "id,payload", sourceQuery: $source_query, sinkDatasourceId: $sink_datasource_id, sinkDatasourceUseEnabled: true, sinkTable: "phase3_sink", mode: "complete-atomic", jobs: 1, maxAttempts: 2, retryBackoffSeconds: 0, automaticRetryEnabled: true}')
...
phase4_create_postgres_datasources "$api_one" "phase3 resilience"
