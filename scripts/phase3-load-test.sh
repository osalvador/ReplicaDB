...
export REPLICADB_WORKER_ONE_MAX_CONCURRENT_RUNS=${PHASE3_LOAD_WORKER_ONE_CAPACITY:-1}
export REPLICADB_WORKER_TWO_MAX_CONCURRENT_RUNS=${PHASE3_LOAD_WORKER_TWO_CAPACITY:-1}
export REPLICADB_WORKER_ONE_ADMISSION_JITTER_MAX=${PHASE3_LOAD_WORKER_ONE_JITTER_MAX:-0ms}
export REPLICADB_WORKER_TWO_ADMISSION_JITTER_MAX=${PHASE3_LOAD_WORKER_TWO_JITTER_MAX:-0ms}
export REPLICADB_WORKER_ONE_ADMISSION_GENERIC_COOLDOWN=${PHASE3_LOAD_WORKER_ONE_COOLDOWN:-0ms}
export REPLICADB_WORKER_TWO_ADMISSION_GENERIC_COOLDOWN=${PHASE3_LOAD_WORKER_TWO_COOLDOWN:-0ms}
...
create_job() {
    ...
    payload=$(jq -n \
        ...
        '{name: $name, sourceDatasourceId: $source_datasource_id, sourceDatasourceUseEnabled: true, sourceTable: "phase3_source", sourceColumns: "id,payload", sinkDatasourceId: $sink_datasource_id, sinkDatasourceUseEnabled: true, sinkTable: $sink_table, mode: "complete-atomic", jobs: 1, maxAttempts: 1, retryBackoffSeconds: 0, automaticRetryEnabled: false}')
...
phase4_create_postgres_datasources "$api_one" "phase3 load $load_seed"
